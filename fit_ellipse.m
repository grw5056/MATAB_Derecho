function [ellipse_t,phi] = fit_ellipse( x,y,axis_handle )
%
...
ellipse_y_r     = Y0 + b*sin( theta_r );
rotated_ellipse = R * [ellipse_x_r;ellipse_y_r];
xCoords=rotated_ellipse(1,1:end-1);
yCoords=rotated_ellipse(2,1:end-1);
xCoords=cat(1,xCoords(end),xCoords(:),xCoords(1));
yCoords=cat(1,yCoords(end),yCoords(:),yCoords(1));
coor = [xCoords(:), yCoords(:)];
angle=zeros(1,length(xCoords)-2);
% Calculate the vectors BA and BC
for l=1:length(xCoords)-2
    BA = sqrt(((coor(l+1,1) - coor(l,1)).^2)+((coor(l+1,2) - coor(l,2)).^2));
    BC = sqrt(((coor(l+1,1) - coor(l+2,1)).^2)+((coor(l+1,2) - coor(l+2,2)).^2));
    AC = sqrt(((coor(l,1) - coor(l+2,1)).^2)+((coor(l,2) - coor(l+2,2)).^2));
    %
    angle(l)=acosd(((BA^2)+(BC^2)-(AC^2))/(2*BA*BC));
end
[minVal_HCR_1,minloc_HCR_1]=min(angle);
angle(minloc_HCR_1)=9999;
[minVal_HCR_2,minloc_HCR_2]=min(angle);
coor=coor(2:end-1,:);
min_points_1=coor(minloc_HCR_1,:);
min_points_2=coor(minloc_HCR_2,:);    
phi=atand((min_points_1(2)-min_points_2(2))/(min_points_1(1)-min_points_2(1)));
